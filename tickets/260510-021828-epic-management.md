...
- [ ] yaml-sh helpers / frontmatter (de)serialize for epic files
- [ ] `resolve_epic <slug>` helper
- [ ] `cmd_epic_new` with --title / --branch / --main-direct / --from-ref + template
- [ ] `cmd_new --epic` augmentation (sets epic_id + base_branch)
- [ ] `cmd_epic_close` epic-branch path (squash + auto-resolve)
- [ ] `cmd_epic_close` main-direct path
- [ ] `cmd_epic_cancel` epic-branch path (drop impl, keep epic body)
- [ ] `cmd_epic_cancel` main-direct path
- [ ] Conflict resolver after `merge --squash -X theirs`
- [ ] Preflight (clean tree, branch existence, closed-already, open linked tickets, dry-run squash)
- [ ] `cmd_epic_list` text + --json
- [ ] `cmd_epic_show` text + --json (full schema for pdh-flow)
- [ ] `cmd_epic` dispatcher + help text
- [ ] Tests: test-epic-happy-path.sh, test-epic-cancel.sh, test-epic-preflight.sh, test-epic-dirty-tree.sh
- [ ] Run tests before closing and pass all tests
- [ ] Run `bash build.sh` to build the project
- [ ] Push and comment on the gist
